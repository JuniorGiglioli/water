...
export class LoginService {
    ...
    getUserInfo() {
        const userInfo = localStorage.getItem( Constants.STORAGE.USER_INFO );
        if ( userInfo ) {
    ...
    public fetchUserInfo() {
        ...
        const promisse = this.requester.get( this.configService.getConfig().getUserInfoUrl() );
    ...
    private logoutAuthorizationServer( accessToken: string ) {
        ...
        if ( userInfo ) {
            const logoutUrl = this.configService.getConfig().getLogoutUrl();

            const url: string = logoutUrl.replace( '__userId__', userInfo['id'] );

            const promisse = this.requester.get( url, null, { 'Accept': 'text/plain' } );

            promisse.then(( response: Response ) => {
                const iframe = window.document.getElementById( 'logoutFrame' );
